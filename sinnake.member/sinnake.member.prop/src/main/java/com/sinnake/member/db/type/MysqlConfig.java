package com.sinnake.member.db.type;
...
public class MysqlConfig implements DbConfigInterface {
	 ...
	 */
	@Autowired
	public MysqlConfig(@Value("#{memberDbProp['member.db.mysql.username']}") String userName
		, @Value("#{memberDbProp['member.db.mysql.url']}") String url
		, @Value("#{memberDbProp['member.db.mysql.password']}") String password
		, @Value("#{memberServerProp['member.dbAes256.key']}") String dbKey) {
