package com.sinnake.prop.db.type;
 ...
 */
@Configuration
@Component("temporaryMySqlConfig")
public class MysqlConfig implements DbConfigInterface {
	 ...
	 */
	@Autowired
	public MysqlConfig(@Value("#{temporaryDbProp['temporary.db.mysql.username']}") String userName
		, @Value("#{temporaryDbProp['temporary.db.mysql.url']}") String url
		, @Value("#{temporaryDbProp['temporary.db.mysql.password']}") String password
		, @Value("#{temporaryServerProp['temporary.dbAes256.key']}") String dbKey) {
