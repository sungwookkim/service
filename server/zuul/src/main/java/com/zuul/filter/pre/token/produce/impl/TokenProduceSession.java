...
public class TokenProduceSession extends TokenProduceDefault {

	public TokenProduceSession() {}

	public TokenProduceSession(String url, String userName, RestTemplate restTemplate, RequestContext requestContext) {
	...
	public void tokenSave(Map<String, Object> resultToken, HttpServletRequest req, HttpServletResponse resp) {
		...
		CookieUtil.setCookie(TokenKey.TOKEN_SAVE_ACCESS_TOKEN.getKey(), null, c -> {
			c.setPath("/");
			c.setMaxAge(0);
			resp.addCookie(c);
		});
		...
		req.getSession().setAttribute(TokenKey.TOKEN_SAVE_ACCESS_TOKEN.getKey(), accessToken);
